...
folder_output = 'output_Diskontinu_CTCS_Polutan_Terhadap_Ruang';
...
for s = 1:numel(scenario)
    ...
    % FTCS untuk mendapatkan level waktu n = 1
    for i = 2:(Mmax-1)
    ...
    % Sumber diskontinu pada n = 1
    if n_source == 1
        F0(m_source) = C_source;
    end

    idx1 = find(n_plot == 1, 1);
    if ~isempty(idx1)
        snapshot(idx1, :) = F0;
    end
    ...
    nama_file = sprintf('ctcs_diskontinu_ruang_skenario_%d.png', s);
