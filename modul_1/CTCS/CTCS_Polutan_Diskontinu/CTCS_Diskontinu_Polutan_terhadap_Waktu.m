...
folder_output = 'output_Diskontinu_CTCS_Polutan_Terhadap_Waktu';
...
for s = 1:numel(scenario)
    ...
    for p = 1:length(m_plot_valid)
        history(1, p) = F00(m_plot_valid(p));
    end
    ...
    nama_file = sprintf('ctcs_diskontinu_waktu_skenario_%d.png', s);
