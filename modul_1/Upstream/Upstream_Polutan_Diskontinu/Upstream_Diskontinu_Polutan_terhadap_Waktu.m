...
% Simulasi adveksi 1 dimensi dengan metode Upstream
...
folder_output = 'output_Diskontinu_Upstream_Polutan_Terhadap_Waktu';
...
for s = 1:numel(scenario)
    ...
    for J = 1:Nmax
        ...
        for i = 2:(Mmax-1)
            if u >= 0
                F(i) = F0(i) - (u * dt / dx) * (F0(i) - F0(i-1));
            else
                F(i) = F0(i) - (u * dt / dx) * (F0(i+1) - F0(i));
            end
        end
    ...
    title(sprintf('%s - Konsentrasi polutan terhadap waktu (Upstream)', scenario(s).nama), ...
    ...
    nama_file = sprintf('upstream_diskontinu_waktu_skenario_%d.png', s);
