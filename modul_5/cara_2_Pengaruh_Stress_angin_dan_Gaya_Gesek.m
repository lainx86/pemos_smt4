...
folder_output = 'Cara_2_Grafik pengaruh stress angin dan gaya gesek dasar';
...
for s = 1:length(scenario)
    ...
    fprintf('%s | Cs=%.4f | CFL=%.4f | W=%.1f | r=%.4f | tau_s=%.6f\n', ...
        scenario(s).nama, Cs, CFL, W, r, tau_s);
    ...
    for n = 1:Nt-1
        ...
        for j = 1:Nx-1
            pembilang   = u(j,n) - g*dt/dx*(Z(j+1,n) - Z(j,n)) + dt/H*tau_s;
            penyebut    = 1 + r*dt/H * abs(u(j,n));
            u(j,n+1)    = pembilang / penyebut;
        end
    ...
    title(ax, {
        sprintf('\\tau_b vs Waktu (Cara 2) - %s (j=%d, x=%.0f m)', ...
    ...
    saveas(fig, fullfile(folder_output, ...
        sprintf('taub_cara2_vs_waktu_skenario_%d.png', s)));
    close(fig);

end

disp('Seluruh grafik tau_b cara 2 selesai dibuat dan disimpan.');