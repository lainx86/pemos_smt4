...
folder_output = 'output_Kontinu_FTCS_Terhadap_Ruang';
...
for s = 1:numel(scenario)
    ...
    fprintf('dx = %.3f m, dt = %.3f s, Ad2 = %.3f m^2/s, m = %d\n', dx, dt, Ad2, m);
    ...
    tv_valid = tv(tv >= n(1) & tv <= n(end));

    if length(tv_valid) < length(tv)
        fprintf('%s: beberapa waktu tv berada di luar domain sehingga tidak digrafikkan.\n', scenario(s).nama);
    end

    history = NaN(length(tv_valid), mmax);
    ...
    for p = 1:length(tv_valid)
        [~, idx_tv] = min(abs(n - tv_valid(p)));
        history(p, :) = F(idx_tv, :);
    end
    ...
    for k = 1:length(tv_valid)
        plot(t, history(k, :), 'LineWidth', 1.2);
        legend_text{end+1} = sprintf('t = %d', tv_valid(k));
    end

    grid on;
    xlabel('Ruang r (meter)', 'FontSize', 12);
    ...
    nama_file = sprintf('ftcs_kontinu_ruang_skenario_%d.png', s);
